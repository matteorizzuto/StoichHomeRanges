...
<!--     * this is related to the individuals having an easier/tougher time finding the resources they need to satisfy their elemental dietary requirements based on the spatial distribution of the plant content of those elements -->
...
```{r year-models-2, echo=TRUE, message=FALSE, warning=FALSE, tidy=TRUE}
...
of telemetry sampling. The following code chunk extracts the summary 
statistics---i.e., median and range---reported for home range size and plant SOI 
stoichiometric ratios within the core area that we report in the Results section
of the manuscript (values not shown).

```{r summary-stats-harerich,echo=TRUE,tidy=TRUE, results='hide'}
hare.rich %>% dplyr::group_by(., SamplingYear) %>% dplyr::summarise(., avgHR = mean(HRsize), maxHR = max(HRsize), minHR = min(HRsize))

summary(hare.rich$HRsize)

stoich50 %>% dplyr::summarise_at(., vars(VAAN_CN:ACRU_NP), list(min = min, median = median, max = max), na.rm=TRUE)
```
...
diagnostic plots. For the top-ranked models in the AICc table at the bottom of this section, we additionally present a graph
...
acruCNtab50 <- gt(acruCNaicRsq) %>% fmt_number(columns = 3:9, decimals = 3) %>%
...
gtsave(acruCNtab50, "../Results/ACRU_CN_50modseltab.tex")
...
vaanCNtab50 <- gt(vaanCNaicRsq) %>% fmt_number(columns = 3:9, decimals = 3) %>% 
  tab_header("Model Selection for Lowbush blueberry C:N") %>% 
    cols_label(Modnames ="Models formula", K = "K", AICc = "AICc", 
             Delta_AICc = html("&Delta; AICc"), ModelLik ="Model Likelihood", 
             AICcWt = "AICc Weight", LL ="Log-Lik", 
             Cum.Wt = "Cumulative Weight", Rsq = html("R<sup>2</sup>"))

gtsave(vaanCNtab50, "../Results/VAAN_CN_50modseltab.tex")
...
acruNPtab50 <- gt(acruNPaicRsq) %>% fmt_number(columns = 3:9, decimals = 3) %>% 
...
gtsave(acruNPtab50, "../Results/ACRU_NP_50modseltab.tex")
...
vaanNPtab50 <- gt(vaanNPaicRsq) %>% fmt_number(columns = 3:9, decimals = 3) %>% 
  tab_header("Model Selection for Lowbush blueberry N:P") %>% 
    cols_label(Modnames ="Models formula", K = "K", AICc = "AICc", 
             Delta_AICc = html("&Delta; AICc"), ModelLik ="Model Likelihood", 
             AICcWt = "AICc Weight", LL ="Log-Lik", 
             Cum.Wt = "Cumulative Weight", Rsq = html("R<sup>2</sup>"))

gtsave(vaanNPtab50, "../Results/VAAN_NP_50modseltab.tex")
...
vaanCPtab50 <- gt(vaanCPaicRsq) %>% fmt_number(columns = 3:9, decimals = 3) %>% 
  tab_header("Model Selection for Lowbush blueberry C:P") %>% 
    cols_label(Modnames ="Models formula", K = "K", AICc = "AICc", 
             Delta_AICc = html("&Delta; AICc"), ModelLik ="Model Likelihood", 
             AICcWt = "AICc Weight", LL ="Log-Lik", 
             Cum.Wt = "Cumulative Weight", Rsq = html("R<sup>2</sup>"))

gtsave(vaanCPtab50, "../Results/VAAN_CP_50modseltab.tex")
...
top.mods[[10]] <- acruNPaicRsq[3,]
top.mods[[11]] <- acruNPaicRsq[4,]

# Red maple C:N ratio top models
top.mods[[12]] <- acruCNaicRsq[1,]
top.mods[[13]] <- acruCNaicRsq[3,]

SumTab50 <-top.mods %>% bind_rows() %>% 
  gt() %>% 
  ...
  tab_row_group(., group = "Red maple N:P top models", rows = c(9:11)) %>% 
  tab_row_group(., group = "Red maple C:N top models", rows = c(12,13)) %>% 
...
gtsave(SumTab50, filename = "../Results/ModSel50kUD_SummaryTable.tex")
...
Lowbush blueberry C:P ratio       Body Weight 
Red maple N:P ratio               Mean, Body Weight
Red maple C:N ratio               Mean, Mean-CV interaction
