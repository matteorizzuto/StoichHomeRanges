...
# code chunk
knit("HomeRangeEstimation.Rmd", tangle = TRUE)
...
> **CODE BELOW WORKS BUT NOT THE BEST TO SHOW THIS**
...
# setup grid points
bl_grid_pts <- bl_grid_points %>% dplyr::select(SiteName,TrapName)

bl_grid_points <- st_transform(bl_grid_pts, st_crs(hares.points))

# convert hare.points to sf
hares.points <- st_as_sf(hares.points)

# extract the UD contours at 50%, 75%, and 90% isopleths
homeRanges.50 <- lapply(1:length(homeRanges), function(x) SpatialPolygonsDataFrame.UD(homeRanges[[x]], level.UD=0.50, level = 0.95))
homeRanges.75 <- lapply(1:length(homeRanges), function(x) SpatialPolygonsDataFrame.UD(homeRanges[[x]], level.UD=0.75, level = 0.95))
homeRanges.90 <- lapply(1:length(homeRanges), function(x) SpatialPolygonsDataFrame.UD(homeRanges[[x]], level.UD=0.90, level = 0.95))
...
for (i in 1:length(loopCIDs)) {
  ...
  aKDE.maps[[i]] <- 
    ...
    tm_shape(icrSf.temp) + 
    tm_lines(col = "id", palette = red.pal, title.col = "% kUD -- ICR", lwd = 2) +
  ...
  # print(hares.kUD.maps[[i]])
  
  # browser()
}

# arrange maps into a single visualization
ICRvMRmaps <- tmap_arrange(ICRvMR.kUD.maps)
...
saveRDS(r, "../Results/akderasters.rds")
# load grid points
bl_grid_pts <- sf::st_transform(bl_grid_points, "+init=epsg:32622")

# load home range area rasters - in a list (95% home range area in hectares)
kernel95 <- readRDS("../Results/akderasters.rds")
...
# save clamped raster
saveRDS(kernel95normz, "../Results/rasternormclamp.rds")
clam <- readRDS("../Results/rasternormclamp.rds")
...
<!-- ## 50% kUD Overlap visualization -->

<!-- The map below allows visualization of the home ranges from the three years of -->
<!-- study and their overlap in an interactive way. We plot the home ranges on a -->
<!-- StDM raster of C:N for blueberry, but other options are available by re the code chunk. -->
```{r kUD-overlap-50, eval=FALSE, include=FALSE, tidy=TRUE}
...
# add to each kUD its eartag and year of sampling

for (i in 1:length(hares.kUD)) {
  hares.kUDhr.50@data[["eartag"]][i] <- as.character(hare.stoich$EarTag[i])
  hares.kUDhr.50@data[["year"]][i] <- as.character(hare.stoich$SamplingYear[i])
}

hares.kUDhr.50@data[["eartag"]] <- as.factor(hares.kUDhr.50@data[["eartag"]])

hares.kUDhr.50@data[["year"]] <- as.factor(hares.kUDhr.50@data[["year"]])


kUD50overlap_map <- tm_shape(vaanCN, bbox = bbox_bldots, name = "Blueberry C:N") +
  tm_raster(style = "cont", palette = "-PRGn", legend.show = TRUE, title = "Blueberry C:N") +
  # tm_shape(vaanCP, bbox = bbox_bldots) +
  # tm_raster(style = "cont", palette = "-PRGn", legend.show = TRUE, title = "Blueberry C:P") +
  # tm_shape(vaanNP, bbox = bbox_bldots) +
  # tm_raster(style = "cont", palette = "-PRGn", legend.show = TRUE, title = "Blueberry N:P") +
  # tm_shape(acruCN, bbox = bbox_bldots) +
  # tm_raster(style = "cont", palette = "-PRGn", legend.show = TRUE, title = "Red Maple C:N") +
  #   tm_shape(acruNP, bbox = bbox_bldots) +
  # tm_raster(style = "cont", palette = "-PRGn", legend.show = TRUE, title = "Red Maple N:P") +
      tm_shape(bl_grid_pts, is.master = T,
                                     name = "Trap Locations") +
  tm_dots(size = 0.1, col = "red") +
     tm_shape(hares.kUDhr.50, name = "Snowshow hare 50% kUD") +
     tm_polygons(col = "id", alpha = 0.7, style = "fixed", title = "Collar Frequency") +
     ...
     tm_layout(legend.outside = TRUE,
               ...
               asp = 1,
               frame = TRUE,
               title = "Snowshoe hare 50% kUD") +
  tmap_options(max.categories = 34) + tm_facets(by = "eartag", as.layers = TRUE)

tmap_mode(mode = "view")

kUD50overlap_map

# tmap_save(filename = "OL_interactiveMap.html")
```



```{r kUD-visual, eval=FALSE, include=FALSE, tidy=TRUE}
# the following custom function gets the estimated percent contours of a kernel UD
# Taken from: https://mgritts.github.io/2016/06/01/visualize-utilization-distribution/
...
# extract volume of kUDs to plot later underneath the isopleths
hares.vol <- getvolumeUD(hares.kUD)

# get 50% and 90% contours from kUD of each hare
hares.kUD.contour <- lapply(hares.kUD, function(x) getContours(x, pct = c(50, 75, 90)))

# allocate empty list to store tmap objects for each map created below
hares.kUD.maps <- vector("list", length(UniqCIDs.bl))

# sort the unique identifiers 
UniqCIDs.bl <- sort(UniqCIDs.bl)

# create a grey scale palette for isopleths, black for 50%, dark gray for 75% and light grey 
# for 90%
grey.pal <- c("#000000", "#4c4c4c", "#a6a6a6") 
...
for (i in 1:length(UniqCIDs.bl)) {
    ...
    lines.temp <- as(hares.kUD.contour[[i]], "SpatialLinesDataFrame") 
    
    # add a crs to the temporary linear object
    crs(lines.temp) <- crs(hares.triangd) 
    
    bbox_temp <- st_bbox(lines.temp) # current bounding box
    ...
    # create a map using tmap and store it in its own slots in the list created
    # above
    hares.kUD.maps[[i]] <- tm_shape(hares.vol[[i]], bbox = bbox_temp) + 
      tm_raster(palette = "-YlOrRd", n = 10) + 
      tm_shape(lines.temp, unit = "km") + 
      tm_lines(col = "id", palette = grey.pal, title.col = "% kUD", lwd = 2) +
      tm_compass(position = c("right", "top")) +
      tm_scale_bar(width = 0.3, 
                   text.size = 1.25,
                   position = c("right", "bottom")) + 
      tm_layout(main.title = paste("Collar Frequency:", UniqCIDs.bl[i]),
                # title = paste("Ear tag:", dots.temp$EarTag),
                legend.outside = TRUE, 
                legend.outside.position = "right",
                legend.text.size = 1.1,
    ...
    rm(lines.temp) # remove the temp linear object to avoid errors
}

# arrange and print the maps
# kUDmaps <- tmap_arrange(hares.kUD.maps, ncol = 7, nrow = 4)
# kUDmaps

# tmap_save(hares.kUD.maps[[i]], filename = "../Results/kUDsSelection_example.png", dpi = 300)
```
