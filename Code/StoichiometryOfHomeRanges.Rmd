...
subtitle: "<h2>Supporting Code</h2>"
...
stoichiometry and home range size, at three kUD isopleths: 50% (the "core 
area"), 75%, and 90%
...
*Vaccinium angustifolium*   Blueberry       VAAN
...
In the "setup" R code chunk below, we also load shapefiles for:

* the locations of all our traps within our four trapping grids, and then 
separate the traps on the study grid from the other three grids
* a shapefile of the Forest Resource Inventory (henceforth, FRI) polygons 
delimiting forest patches

both these shapefiles will be used later in this notebook, when producing maps
showing the spatial distribution of our points.
...
--- e.g. either binary (preferred) or fromtheir respective development sources. 
...
> Do note, however, that upgrading `GDAL` to its 3.x version or `PROJ` to its
6.x version may compromise parts of the R spatial code below. Most of these 
issues are widespread throughou the R spatial community, as mentioned above, 
and will likely take some time to be resolves. As such, please check your R 
spatial packages configuration before attempting to recompile this notebook.

```{r pkg-file-load, echo=TRUE, tidy=TRUE}
...
field-->
```{r child=here::here("TriangulationError.Rmd")}
...
```{r child=here::here("razimuthTriangulation.Rmd")}
...
```{r child=here::here("aKDE_HomeRangeEstimation.Rmd")}
...
```{r child=here::here("75kUDHRModels.Rmd")}
...
```{r child=here::here("90kUDHRModels.Rmd")}
...
```{r child=here::here("AdditionalPlots.Rmd")}
